...
if __name__ == '__main__':
    ...
    async def test():
        ...
        input_text = "请给我推荐几个关于 Mamba原理 的视频。"
        ...
        finish = False
        test = 20
        async for event in chain.astream_events(input_all, config, version="v2"):
            kind = event["event"]
            if kind == "on_retriever_end":
                # yield event["data"]["output"]
                # print(event["data"]["output"])
                # lst = event["data"]["output"]
            ...
            if kind == "on_chat_model_stream":
                ...
                # if generate == "generate" and not finish:
                #     # yield chunk["content"]
                #     print(chunk.content, end="", flush=True)
                #     finish = chunk.response_metadata
                    # if finish is not None and finish["finish_reason"] == "stop":
                    #     break
            # if test:
            #     test -= 1
            #     print(event)
